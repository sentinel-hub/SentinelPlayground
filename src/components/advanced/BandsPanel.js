    ...
    for (let i in Store.current.layers) {
      if (Store.current.layers.hasOwnProperty(i)) {
        ...
        document.getElementById('o' + l).innerHTML =
          name !== 'NULL'
            ? `<div style="background-color: ${channel.color}" title="${channel.desc}">${name.replace('B', '')}</div>`
            : ''
      }
    }
  }
    ...
    if (_.includes(Store.current.layers, 'NULL')) {
      return (
        <div id="warning" className="notification">
          <i className="fa fa-warning" />You need to fill all three channels to provide Sentinel imagery.
        </div>
      )
    }
  }
      ...
      <div id="colorsWrap">
        ...
        <div className="colorsContainer" ref="colorsHolder" id="colorsHolder">
          {Store.current.channels.map((channel, i) => (
            <div key={i} title={channel.desc} style={{ backgroundColor: channel.color }} data-name={channel.name}>
