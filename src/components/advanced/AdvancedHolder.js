...
class AdvancedHolder extends React.Component {
    ...
    toggleMode = () => {
        ...
        let isEvalScriptFromLayers = (Store.current.evalscript === btoa("return [" + _.values(Store.current.layers).join(",") + "]") || Store.current.evalscript === '')
        if (isEvalScriptFromLayers) {
            Store.setEvalScript(this.isScriptView() ? btoa("return [" + _.values(Store.current.layers).join(",") + "]") : '')
            Store.refresh()
        }
    }
