...
export default class PlaygroundLegend extends React.Component {
  ...
  componentWillMount() {
    this.content = null;
    let link =
      this.props.link + `&format=application/json&timecachebusting=${new Date().valueOf()}`;
        ...
        let _div = json.entries.map(entry => {
          ...
          return (
            <div key={label} className="PlaygroundLegend_Row">
              <div className="PlaygroundLegend_Color" style={{ backgroundColor: color }} />

              <div className="PlaygroundLegend_EntryLabel">{label}</div>
  ...
  render() {
    ...
    return <div>{hasContent ? content : ''}</div>;
  }
}
